# Fluffy Enigma — Setup & Operations Guide
...
- Permissions are atomic strings used in route + UI guards, e.g.
  `dashboard.view`, `shortlinks.view`, `shortlinks.manage`, `shortlinks.view_all`,
...
- Starter roles seeded by `PermissionSeeder` (idempotent, runs on every boot):
  ...
  - `editor` — dashboard + short-link view/manage.
  - `viewer` — dashboard + short-link view.
...
A single **Overview** page (gated by `dashboard.view`) showing home-page traffic
(total hits, unique / returning visitors, repeat hits, today's count) and short-link
activity (counts + top links, scoped to what the user may see).
...
generates `APP_KEY`, migrates, seeds the baseline roles & permissions, and
...
2. Put that hostname in `deploy/config.env` as `DOMAIN="app.example.com"`.
...
|----------|---------|-------|
| `APP_NAME` | `Fluffy Enigma` | Overridden by the portal name set during onboarding |
